    ...
    <form action="{{ route('back.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="genre" class="form-select" required>
                ...
                <option value="homme" {{ old('genre', $article->genre) == 'homme' ? 'selected' : '' }}>Homme</option>
                <option value="femme" {{ old('genre', $article->genre) == 'femme' ? 'selected' : '' }}>Femme</option>
                <option value="neutre" {{ old('genre', $article->genre) == 'neutre' ? 'selected' : '' }}>Neutre</option>
            </select>
        </div>
